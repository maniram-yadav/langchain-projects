...
class DisplayResultStreamlit:
    ...
    def display_result_on_ui(self):
        ...
        elif usecase == "Politics News":
            frequency = self.user_message
            with st.spinner("Fetching and summarizing Politics news... ⏳"):
                result = graph.invoke({"messages": frequency})
                try:
                    # Read the markdown file
                    POLITICS_NEWS_PATH = f"./PoliticsNews/{frequency.lower()}_summary.md"
                    with open(POLITICS_NEWS_PATH, "r") as file:
                        markdown_content = file.read()

                    # Display the markdown content in Streamlit
                    st.markdown(markdown_content, unsafe_allow_html=True)
                except FileNotFoundError:
                    st.error(f"News Not Generated or File not found: {POLITICS_NEWS_PATH}")
                except Exception as e:
                    st.error(f"An error occurred: {str(e)}")
                    
                
                with open(POLITICS_NEWS_PATH, 'r') as f:
                    st.download_button(
                        "💾 Download Summary",
                        f.read(),
